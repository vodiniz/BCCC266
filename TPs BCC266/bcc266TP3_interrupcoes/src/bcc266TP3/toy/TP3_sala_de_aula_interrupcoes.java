...
public class TP3_sala_de_aula_interrupcoes {
		...
		final int tamanhoPrograma = 10001;//qde de instru��es
		final int tamanhoProgramaTI = 101;//qde de instru��es tratador de interrup��es
		...
		void maquina(int PC, String programa){
			...
			while(opcode!=-1){
				...
				if(opcode!=-1){
					...
					System.out.println("Custo at� o momento do programa em execu��o: " + custo);
					System.out.println("Hits e Misses at� o momento - C1 hit | C1 miss | C2 hit | C2 miss: " + hitC1+ " | "+ missC1 + " | "+hitC2 + " | "+ missC2);
					
					switch (opcode){
						//levar para cache1 dados externos
						case 0:{
							System.out.println("Levando dados para cache e gerando uma interrup��o para isto !!! ");
							//gerar 1 ou n interrup��es no barramento
				...
				//retiram da pilha uma interrup��o
				if(programa.equals("programa") && !pilhaInterrupcoes.isEmpty()){
					pilhaInterrupcoes.remove(0);//removendo uma interrup��o para ser tratada
					System.out.println();
			...
			System.out.println("Interrupcoes ainda n�o tratadas: " + pilhaInterrupcoes.size());
